...
{
    set_override_cursor(event.position().y() < 16 ? m_skin.cur_titlebar() : Gfx::StandardCursor::Arrow);
...
void AmpWidget::paint_event(GUI::PaintEvent& event)
{
    ...
    bool light_up_mono_stereo_light = true;
    switch (m_play_state) {
    ...
    case Player::PlayState::NoFileLoaded:
        ...
        light_up_mono_stereo_light = false;
        break;
    }

    // Draw Stereo / Mono lights
    if (!light_up_mono_stereo_light) {
