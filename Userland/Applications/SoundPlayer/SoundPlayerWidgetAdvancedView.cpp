...
void SoundPlayerWidgetAdvancedView::play_state_changed(Player::PlayState state)
{
    ...
    case PlayState::NoFileLoaded:
        ...
        m_play_button->set_enabled(false);
        m_stop_button->set_enabled(false);
        m_playback_progress_slider->set_enabled(false);
        break;
...
{
    auto visualization = dynamic_cast<Visualization*>(m_visualization.ptr());
    visualization->set_buffer(buffer);
    visualization->set_samplerate(sample_rate);
