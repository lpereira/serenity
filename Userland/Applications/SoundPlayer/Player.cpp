    ...
    , m_playback_manager(audio_client_connection)
{
    ...
    m_playback_manager.on_load_sample_buffer = [&](Audio::Buffer&) {
...
void Player::play_file_path(StringView path)
{
    ...
    if (!Core::File::exists(path)) {
        GUI::MessageBox::show(nullptr, String::formatted("File \"{}\" does not exist", path), "Error opening file", GUI::MessageBox::Type::Error);
        return;
    }
